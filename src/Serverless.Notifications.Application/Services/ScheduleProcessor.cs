...
public class ScheduleProcessor : IScheduleProcessor
{
    ...
    public async Task ProcessQueueAsync()
    {
        while (true)
        {
            ...
            var messages = await _cloudQueueStorage.ReceiveMessagesAsync(SCHEDULE_QUEUE_NAME);
            ...
            var peekedMessages = await _cloudQueueStorage.PeekMessagesAsync(SCHEDULE_QUEUE_NAME);

            if (await _cloudQueueStorage.GetApproximateMessagesCountAsync(SCHEDULE_QUEUE_NAME) == 0 ||
    ...
    private async Task ProcessMessageAsync(QueueMessage message)
    {
        ...
        var notification = JsonConvert.DeserializeObject<Notification>(message.MessageText);
